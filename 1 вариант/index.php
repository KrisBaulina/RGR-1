...
echo "<H1>"."Прайс-лиcт на принтеры:"."</H1>";

function printersCompare ($printer1,$printer2){
	if ($printer2['prices']['amount']>$printer1['prices']['amount']){
		return 1;
	} else if ($printer2['prices']['amount']<$printer1['prices']['amount']) {
		return  -1;
	} else {	
		return 0;
	}
...
usort ($data, 'printersCompare');

foreach ($data as $printer){
	$photo = htmlspecialchars($printer['mainPhoto']['url']);
	$link = htmlspecialchars($printer['link']); 
	...
	<table>
		<tr> 
			...
			<td>
				<ul type="circle">
					<br>
					<li><div>Марка принтера: <?= htmlspecialchars($printer['name']) ?></div></li>
					<li><div>Описание: <?= htmlspecialchars($printer['description']) ?></div></li>
					<li><div>Производитель: <?= htmlspecialchars($printer['vendor']) ?></div></li>
					<li><div>Цена: <?= htmlspecialchars(number_format(($printer['prices']['amount']),2,',',' ')) ?> руб.</div></li>
					<li><div>Рейтинг: <?= htmlspecialchars($printer['rating']) ?></div></li>
					<li><div>Количество отзывов: <?= htmlspecialchars($printer['reviewsCount']) ?></div></li>
